...
def prepare(crossover, target, modules=None):
    ...
    if modules:
        ...
        # The PE half of ntdll carries the Chromium/CEF command-line patch, so
        # it comes from the build too when one was supplied.
        replacements["ntdll.dll"] = modules / "dlls/ntdll/x86_64-windows/ntdll.dll"
    ...
    # must be co-located, or loading fails with c0000135. When no build was
    # supplied the PE half is taken from CrossOver unchanged (only the Unix
    # half is patched then), so the pair still holds.
    if "ntdll.dll" not in replacements:
