...
（未做任何改动），**7 个实体文件**。这 7 个里 **5 个是被我们替换的**：
...
lib/wine/x86_64-windows/ntdll.dll         ← 必须存在，但内容未改（见 5.2）
LOCAL_ONLY.txt                            ← 说明文件
```
...
所以结论修正为：**这份 `ntdll.dll` 是必需的，只是内容不需要改。**

**但也故意不换成自建版本**：本仓库的构建来自上游 Wine 源码，而 CrossOver 的
`ntdll.dll` 带 CodeWeavers 自己的补丁，换掉会丢功能。脚本现在会从
CrossOver 原样拷入，并在注释里写明原因。
